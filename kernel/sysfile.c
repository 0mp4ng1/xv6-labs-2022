...
sys_munmap(void)
{
  ...
  for (int i = 0; i < MAXMMAP; i++)
  {
    if (p->vma[i].isUsed == 1 && p->vma[i].addr <= addr && addr < p->vma[i].end)
    {
      ...
      if (p->vma[i].flags == MAP_SHARED)
      {
        ...
        writei(p->vma[i].file->ip, 1, addr, addr - p->vma[i].addr, length);
      ...
      else if (p->vma[i].end == addr + length)
        p->vma[i].end = addr + length;
...
int mmaphandler(uint64 va)
{
  ...
  for (i = 0; i < MAXMMAP; i++)
  {
    if (p->vma[i].isUsed == 1 && va >= p->vma[i].addr && va < p->vma[i].end)
      break;
  }
  if (i == MAXMMAP)
    return -1;
  ...
  // read the file date into that page
  memset(mem, 0, PGSIZE);
  begin_op();
  ilock(file->ip);
  if (readi(file->ip, 0, (uint64)mem, va - addr, PGSIZE) < 0)
  {
    iunlock(file->ip);
    end_op();
    return -1;
